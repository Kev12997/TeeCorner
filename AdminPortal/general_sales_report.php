...
<?php
    ...
    if($lenght == 'day'){
        $sql = "SELECT* FROM shirt NATURAL JOIN orders NATURAL JOIN cart WHERE order_date = '$date'";
        $result = mysqli_query($conn,$sql);
    }elseif($lenght=='week'){
        $sql = "SELECT* FROM shirt NATURAL JOIN orders NATURAL JOIN cart WHERE order_date >= subdate('$date', INTERVAL 1 week) AND order_date <= '$date'";
        $result = mysqli_query($conn,$sql);
    }elseif($lenght=='month'){
        $sql = "SELECT* FROM shirt NATURAL JOIN orders NATURAL JOIN cart WHERE order_date >= subdate('$date', INTERVAL 1 month) AND order_date <= '$date'";
        $result = mysqli_query($conn,$sql);
    }
?>
<h1>
        ...
        <table class="table">
          <thead>
            <tr>
              ...
              <th>Cost</th>
              <th>Price</th>
              <th>Total Revenue From Order</th>
            </tr>
          </thead>
          <tbody>
            <?php
