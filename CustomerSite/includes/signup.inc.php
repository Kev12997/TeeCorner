...
if (isset($_POST['submit']) ){
    ...
    if (empty($email) || empty($first) || empty($last) || empty($password) || empty($birthday)){
        ...
        if(!preg_match("/^[a-zA-Z]*$/", $first) || !preg_match("/^[a-zA-Z]*$/", $last) ){
            header("Location: ../create_account.php?signup=invalid");
            exit();
        } else {
            ...
            if(!filter_var($email, FILTER_VALIDATE_EMAIL)){
                ...
                if($resultCheck > 0){ // email was already in database
                    ...
                    $sql = "INSERT INTO users(first_name, last_name, email, pwd, permission, date_of_birth, age) VALUES ('$first', '$last', '$email', '$hashedPassword', 'CUSTOMER','$birthday',$age);";
                    ...
                    {
                        header("Location: ../create_account.php?signup=success");//if email field where empty, user can still finish account creation
                        exit();
